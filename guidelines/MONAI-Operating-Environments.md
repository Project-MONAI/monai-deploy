...
differences between the different operating environments where MONAI components
are executed. Developers working on different software modules in MONAI MAY use
this specification as a guideline when implementing software for different use
...
**Hardware:**  Laptop with minimum of 8GB RAM, 8 logical core, no GPU in the
minimum environment.  

**Applications:**  Different tools include a mix of development tools, such as 
VSCode, and clinical research tools like jupyterlab, OHIF viewer, python 
...
A single multi user system, where developers, data scientists and clinicians are
collaborating on experimental software versions and new algorithms. 

**Users:** Developers, data scientists, IT administrators, and clinicians.

**Operating System:**  A native linux (RedHat, Ubuntu) server environment, where
...
isolate different sw versions from each other to avoid version conflicts.  
...
**Operating System:**  A native linux (RedHat, Ubuntu) server environment.

**Hardware:**  Large workstation, dedicated server or dedicated VM on a shared
system. CUDA support. Larger HW configurations (GPU, CPU and memory) compared to
the Integration environment.  **Orchestration:** Kubernetes based container
orchestration.  
...
A single approved version of each application is available. Application versions
and deployment in this environment SHALL be managed by Kubernetes. Applications
involved in the end to end workflow are approved by the institution change
control board and managed by experienced IT administrators.
...
clinical image viewers.  **Availability:** Highly available, varying from 99%
